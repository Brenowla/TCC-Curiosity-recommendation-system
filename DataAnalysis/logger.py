...
class Logger(object):
    ...
    def __init__(self, outputDir=""):
        ...
        self._log_file_dict = {'log' : open(self._output_dir + '/log', 'a')}
        
        #self._mutex_done = threading.Lock()
        #self._mutex_log = threading.Lock()

        self._done_file = open(self._output_dir + '/key.done', 'a')
    ...
    def add_log(self, d):
        ...
        """      
        for i, j in d.items():
            self._log_file_dict[i] = open(self._output_dir + "/" + j, 'a')
    ...
    def log_warn(self, k, m, lfk='log'):
        ...
        self._log_file_dict[lfk].write( str([time.strftime('%Y-%m-%d %H:%M'), k, m ]) + '\n' )
