...
def plot_KDE(kde,values,path_save):
    plt.xlabel('Stimulus Degree')
    plt.ylabel('Distribution')
    plt.rcParams.update({'font.size': 12, 'font.family': 'sans-serif'})
    plt.grid(True,axis='both',linestyle=':')
    plt.plot(kde.support, kde.density,'k--',lw=1.5)
    plt.hist(values,bins=50,alpha=0.25,density=True,facecolor='green')
    plt.draw()
...
def plot_U_shaped(values,C,A,B,Pi,path_save_fig):
    ...
    plt.hist(x_values,bins=50,alpha=0.25,facecolor='green')
    plt.draw()
...
def estimate(A,B,Pi,values,C,niter=1000,precision=1E-5):
    ...
    while k > precision and count < niter:
        ...
        W[:,:]=0
        for j in range(C):
            W[:,j]=Pi[j]*beta.pdf(values,A[j],B[j])
...
def test(values,A,B,Pi,C):
    ...
    for j in range(C):
        ...
        try:
            ...
            beta_mix=np.concatenate((beta_mix,np.zeros(np.int(round(values.size*Pi[j])))))
    n=values.size
